...
with col1:
    logo_path = resource_path("axis_logo.jpg")
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
    "Cronología y Distribución", "Análisis por Correlativo", "Análisis por Proceso", "Análisis por Operario", "Evolución Temporal", "Eficiencia Operarios", "Eficiencia Proceso"
])
#------------------------
...
#------------------------
# Pestaña 7: Eficiencia Proceso
#------------------------
with tab7:
    st.subheader("Distribución de Tiempos por Proceso - Campana de Gauss")

    procesos_unicos = sorted(df['Proceso'].unique())

    if len(procesos_unicos) == 0:
        st.warning("No hay procesos disponibles.")
        st.stop()

    # Calcular métricas para títulos
    subplot_titles = []
    for proceso in procesos_unicos:
        df_proc = df[df['Proceso'] == proceso]
        tiempos = df_proc[COL_T_REAL_MIN].dropna()
        if len(tiempos) > 0:
            mean_t = tiempos.mean()
            std_t = tiempos.std()
            subplot_titles.append(f"{proceso.upper()}<br>Media: {format_time_from_minutes(mean_t)} | Desv: {format_time_from_minutes(std_t)}")
        else:
            subplot_titles.append(f"{proceso.upper()}<br>Sin Datos")

    # Determinar layout: 2 columnas
    cols = 2
    rows = (len(procesos_unicos) + cols - 1) // cols
    fig = make_subplots(rows=rows, cols=cols, subplot_titles=subplot_titles, shared_xaxes=False, shared_yaxes=False,
                        horizontal_spacing=0.05, vertical_spacing=0.02)

    for i, proceso in enumerate(procesos_unicos):
        df_proc = df[df['Proceso'] == proceso].copy()
        tiempos_min = df_proc[COL_T_REAL_MIN].dropna()
        if len(tiempos_min) < 2:
            continue

        # Convertir a segundos para plotting
        tiempos_sec = tiempos_min * 60
        mean_min = tiempos_min.mean()
        std_min = tiempos_min.std()

        # Histograma normalizado (en segundos)
        counts, bins_sec = np.histogram(tiempos_sec, bins=20, density=True)
        x_hist_sec = (bins_sec[:-1] + bins_sec[1:]) / 2

        row = i // cols + 1
        col = i % cols + 1

        # Bar plot del histograma
        fig.add_trace(go.Bar(
            ...
            opacity=0.8,
            hovertemplate='<b>Tiempo:</b> %{customdata}<br><b>Densidad:</b> %{y:.4f}<extra></extra>',
            customdata=[format_time_from_minutes(x/60) for x in x_hist_sec]
        ), row=row, col=col)

        # Curva Gaussiana
        mu_sec = mean_min * 60
        sigma_sec = std_min * 60
        x_gauss_sec = np.linspace(tiempos_sec.min(), tiempos_sec.max(), 200)
        y_gauss = (1 / (sigma_sec * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((x_gauss_sec - mu_sec) / sigma_sec)**2)
        ...
        fig.add_trace(go.Scatter(
            ...
            showlegend=False,
            hovertemplate='<b>Tiempo:</b> %{customdata}<br><b>Densidad Gauss:</b> %{y:.4f}<extra></extra>',
            customdata=[format_time_from_minutes(xs/60) for xs in x_gauss_sec]
        ), row=row, col=col)

        # Configurar ticks del eje X en hh:mm:ss
        min_sec = tiempos_sec.min()
        max_sec = tiempos_sec.max()
        range_sec = max_sec - min_sec

        if range_sec > 3600:  # Más de 1 hora de rango
            tick_interval = 3600  # Cada hora
        elif range_sec > 900:  # Más de 15 min
            tick_interval = 1800  # Cada 30 min
        else:
            tick_interval = 300  # Cada 5 min

        tick_vals = []
        current = int(min_sec // tick_interval * tick_interval)
        while current <= max_sec:
            tick_vals.append(max(current, int(min_sec)))
            current += tick_interval

        tick_text = [format_time_from_minutes(tick / 60) for tick in tick_vals]

        fig.update_xaxes(
            title_text='Tiempo Real (hh:mm:ss)',
            row=row, col=col,
            tickmode='array',
            tickvals=tick_vals,
            ticktext=tick_text,
        ...
        fig.update_yaxes(
            title_text='Densidad Probabilidad',
            row=row, col=col,
            showgrid=True,
            gridcolor='lightgray'
        )
    ...
    fig.update_layout(
        height=rows * 450,
        title={
            "text": "<b>Distribución de Tiempos por Proceso - Análisis Estadístico Gaussiano</b>",
            "x": 0.5,
            "xanchor": "center",
            "font": {"size": 18}
        },
    ...
    st.markdown("---")
    st.markdown("**Interpretación:** Cada gráfico muestra la distribución real de tiempos de ejecución de los procesos (histograma azul) ajustada a una distribución normal teórica (línea roja). Los títulos incluyen la media y desviación estándar para cada proceso, permitiendo identificar variabilidad y oportunidades de mejora en la eficiencia.")

#------------------------
# Fin Pestaña 7: Eficiencia Proceso
#------------------------
